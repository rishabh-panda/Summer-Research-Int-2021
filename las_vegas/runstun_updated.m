...
if (NO_SPARSIFICATION==1)
  ...
  grid_x = 50.0; % 20 m x 4 m for ERS-1 (slant res. x azimuth res.)
  grid_y = 250.0;
  IDX1    = sparsify(X,Y,[grid_x,grid_y], 1./F,0);%
...
if (~isempty(points_with_no_arcs))
    C(:,points_with_no_arcs) = [];
end
...
modelled_phase_uw = zeros(NIFG,NPS);
for p=1:NPS
  % --- Report progress ---------------------------------
  if (mod(p-1,100)==0)
...
IDX_closest = dsearchn([X_REF,Y_REF], TRI, [this_X, this_Y]);
  ...
  % --- This is a point in the reference network -----------------------
    ...
    modelled_phase_uw(:,p) = B1*[ref_topo; ref_defo]; % phase convertion matrix B = [h2p v2p]; expect zero at IDX_refpnt
  end
end
disp(' ...done');
disp(' ');

% Let's plot the modelled phase (unwrapped) at all points (REF + other pts)
% in the first two interferograms

figure(3)
subplot(1,2,1)
plotps(X, Y, modelled_phase_uw(1,:));
subplot(1,2,2)
plotps(X, Y, modelled_phase_uw(2,:));
title('Unwrapped modelled phase')

% Let's plot the modelled phase (wrapped) at all points (REF + other pts)
% in first four interferograms
figure(4)
for i=1:4
subplot(2,2,i)
plotps(X, Y, wrap(modelled_phase_uw(i,:)),[-pi pi]); % Wrapped between -pi to pi
end
title('Wrapped modelled phase')

% --- Threshold good estimates -----------------------------------------
disp(' ');
disp('Thresholding on a posteriori variance factor');
IDX_ok = find(allvarf<2.0);
% IDX_ok = find(allvarf<0.5);
